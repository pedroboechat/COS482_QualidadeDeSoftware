...
export default class NotifyMixin extends Vue {
  ...
  public getErrorMessageFromResponse(error: any): any {
    const paramLength: number = error.response.headers['x-cos482app-param-length'];
    if (!paramLength || paramLength == 0) {
      return this.$t(error.response.headers['x-cos482app-error']);
    }

    if (paramLength == 1) {
      return this.$t(error.response.headers['x-cos482app-error'], {
        param: decodeURIComponent(error.response.headers['x-cos482app-param'].replace(/\+/g, ' ')),
      });
    }

    const params = {};
    for (let i = 0; i < paramLength; i++) {
      params['param' + i] = decodeURIComponent(error.response.headers['x-cos482app-param' + i].replace(/\+/g, ' '));
    }
    return this.$t(error.response.headers['x-cos482app-error'], params);
  }

  public getAlertMessageFromResponse(response: any): any {
    return this.$t(response.headers['x-cos482app-alert'], {
      param: decodeURIComponent(response.headers['x-cos482app-params'].replace(/\+/g, ' ')),
    });
  }
}
